...
int reemplazar (tipo_lista l, int e, int p){
	if(!(es_vacia(l))){
		int k=obtener(l,p);
		nodo cursor = l->cabeza;
		for ( int i = 0; i < p ; i++ ){
			cursor = cursor->siguiente;
		};
		cursor->valor = e;
...
void intercambiar (tipo_lista l, int p1, int p2){
	if(!(es_vacia(l))){
		nodo cursor1 = l->cabeza;
		for ( int i = 0; i < p1 ; i++ ){
			cursor1 = cursor1->siguiente;
		};
		int k = cursor1->valor ;
		nodo cursor2 = l->cabeza;
		for ( int i = 0; i < p2 ; i++ ){
		...
		printf("La lista esta vacia y no se pueden intercambiar elementos");
	}
}